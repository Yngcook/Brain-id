...
class EEGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        complex_spectrum = np.fft.fft(minute_data, axis=-1)
        magnitude_spectrum = np.abs(complex_spectrum)
        phase_spectrum = np.angle(complex_spectrum)

        window = np.concatenate([magnitude_spectrum, phase_spectrum], axis=1)
...
class CNNModel(pl.LightningModule):
    def __init__(self, d_model=256, input_channels=64, learning_rate=None, dropout_prob=0.5, num_classes=90):
    ...
    def forward(self, data):
        ...
        data = F.avg_pool1d(data, kernel_size=data.size()[2:])
...
accelerator='gpu',
max_epochs=5,
logger=pl.loggers.CSVLogger("path"),
)
